...
@router.post("/sincronizar/drive/csv-test")
def sincronizar_desde_drive_csv_test(
    req: DriveSyncRequest,
    current_user: dict = Depends(get_current_user)
):
    try:
        ...
        # 2. Ejecutar la lógica de leer_carpetas para actualizar listado_meses.json (filtrando por mes)
        from leer_carpetas import listar_dias_mes
        meses_nuevos = listar_dias_mes(target_month=target_month)
        ...
        # 3. Invocar al helper de prueba de CSV
        from prueba_csv import obtener_hojas_csv_test
        errors = obtener_hojas_csv_test(
            target_month=target_month,
            target_date=target_date,
            force_overwrite=req.overwrite
        )
        ...
        print("[CSV Test] Abriendo conexion dedicada de escritura a Neon...")
        raw_conn = psycopg2.connect(
            dbname="neondb",
            user="neondb_owner",
            password="npg_pPVueS4skO8j",
        ...
        try:
            ...
            db_write.close()
            print("[CSV Test] Conexion dedicada cerrada.")

        return {
            "status": "success",
            "message": "Sincronización de prueba CSV completada exitosamente.",
            "errors": errors
        }
    except Exception as e:
        import traceback
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Error en prueba CSV: {str(e)}"
        )
