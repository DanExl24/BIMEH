<template>
  <div class="glass-panel p-5 sm:p-7 rounded-3xl h-full flex flex-col justify-between hover:border-purple-500/40 transition-all duration-300 group border border-darkBorder shadow-xl">
    <div class="space-y-3.5">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <span class="text-xs text-slate-400">Hoja de vida y novedades por cédula</span>
        </div>
      </div>

      <p class="text-xs text-slate-400 leading-relaxed">
        Genera la hoja de vida operacional e historial de novedades detallado de un integrante por número de cédula militar.
      </p>

      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1 bg-darkBg/60 p-3.5 rounded-2xl border border-darkBorder/60">
        <div class="space-y-1">
          <label class="text-[11px] uppercase font-bold text-slate-300 tracking-wider">Cédula Militar:</label>
          <input 
            v-model="cedulaInput" 
            type="number"
    ...
    <div class="grid grid-cols-3 gap-2 mt-6 pt-4 border-t border-darkBorder/60">
      <button 
        ...
        class="py-2.5 px-2 bg-emerald-500/15 hover:bg-emerald-500/25 disabled:opacity-40 disabled:cursor-not-allowed border border-emerald-500/30 rounded-xl text-xs font-bold text-emerald-300 flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-sm active:scale-95 select-none"
      >
      ...
      <button 
        ...
        class="py-2.5 px-2 bg-cyan-500/15 hover:bg-cyan-500/25 disabled:opacity-40 disabled:cursor-not-allowed border border-cyan-500/30 rounded-xl text-xs font-bold text-cyan-300 flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-sm active:scale-95 select-none"
      >
      ...
      <button 
        ...
        class="py-2.5 px-2 bg-slate-800 hover:bg-slate-700 disabled:opacity-40 disabled:cursor-not-allowed border border-slate-700 text-slate-300 hover:text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-sm active:scale-95 select-none"
      >
