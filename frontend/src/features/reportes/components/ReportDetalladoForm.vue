<template>
  <div class="glass-panel p-5 sm:p-7 rounded-3xl flex flex-col justify-between hover:border-cyan-500/40 transition-all duration-300 group border border-darkBorder shadow-xl">
    <div class="flex flex-col lg:flex-row lg:items-start justify-between gap-5">
      <div class="space-y-3.5 flex-1">
        ...
        <div>
          <h3 class="text-sm font-bold text-slate-100 uppercase tracking-wide">
            Reporte Detallado de Personal (Parte Oficial)
          </h3>
          <p class="text-xs text-slate-400 mt-1.5 leading-relaxed">
            Exporta el parte oficial completo con cédula, nombres, subnovedad, descripción y fechas exactas. Permite filtrar por todos los meses (anual), un mes específico o un día específico.
          </p>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-3 w-full lg:w-72 bg-darkBg/60 p-4 rounded-2xl border border-darkBorder/60 shrink-0">
        <div class="flex flex-col gap-1">
          <label class="text-[11px] uppercase font-bold text-slate-300 tracking-wider">Período a Exportar:</label>
          <select 
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-6 pt-4 border-t border-darkBorder/60">
      <button 
        type="button"
      ...
      >
        ...
        <span>Excel ({{ detalladoLabel }})</span>
      </button>
      <button 
        type="button"
      ...
      >
        ...
        <span>PDF ({{ detalladoLabel }})</span>
      </button>
      <button 
        type="button"
        @click="reportStore.downloadReport(detalladoCsvUrl, `Reporte Detallado - ${detalladoLabel}`, 'csv')"
        class="flex items-center justify-center gap-2 text-center px-4 py-2.5 rounded-xl text-xs font-bold uppercase transition-all duration-200 border cursor-pointer select-none shadow-sm bg-slate-800 border-slate-700 text-slate-300 hover:text-white hover:border-slate-500 active:scale-95"
      >
        <FileCode class="w-4 h-4" />
        <span>CSV ({{ detalladoLabel }})</span>
      </button>
    </div>
  </div>
</template>
...
const detalladoSelectedMonth = ref(props.defaultMonth || 'MAYO')
...
const detalladoFilteredDates = computed(() => {
  const monthNum = MONTH_TO_NUMBER[detalladoSelectedMonth.value]
