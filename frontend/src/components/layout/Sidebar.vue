<template>
  <div>
    ...
    <div 
      v-if="isOpen" 
      @click="$emit('close')"
      class="fixed inset-0 bg-darkBg/80 backdrop-blur-sm z-30 md:hidden transition-opacity duration-300"
    ></div>

    <!-- Navigation Sidebar / Drawer -->
    <aside 
      class="w-64 bg-darkCard/95 border-r border-darkBorder flex flex-col h-screen fixed left-0 top-0 z-40 transition-transform duration-300 md:translate-x-0"
    ...
    >
      ...
      <div class="h-[calc(4rem+env(safe-area-inset-top,0px))] pt-[env(safe-area-inset-top,0px)] flex items-center justify-between px-6 border-b border-darkBorder">
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-md font-bold tracking-wider text-slate-100 uppercase">BIMEJ 12</h1>
            <p class="text-[10px] text-cyan-400 font-semibold tracking-widest uppercase">Reportes Personal</p>
          </div>
        </div>
        ...
        <button 
          @click="$emit('close')"
          class="md:hidden text-slate-400 hover:text-slate-100 p-1.5 rounded-lg hover:bg-darkBorder/40 transition-colors"
        >
          <X class="w-5 h-5" />
        </button>
      </div>
