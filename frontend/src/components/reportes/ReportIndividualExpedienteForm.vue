<template>
  <div class="glass-panel p-5 sm:p-7 rounded-3xl flex flex-col justify-between border-cyan-500/40 bg-gradient-to-br from-cyan-950/20 via-darkPanel to-blue-950/20 hover:border-cyan-400/60 transition-all duration-300 group md:col-span-2 shadow-xl shadow-cyan-950/20">
    <div class="flex flex-col md:flex-row md:items-start justify-between gap-5">
      <div class="space-y-3.5 flex-1">
        <div class="flex items-center gap-3">
          ...
          <span class="px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider bg-cyan-500/20 text-cyan-300 border border-cyan-500/40">
            Exclusivo Novedades
          </span>
        </div>
        <div>
      ...
      <div class="flex flex-col gap-3 w-full md:w-72 bg-darkBg/60 p-4 rounded-2xl border border-darkBorder/60">
        <div class="flex flex-col gap-1">
          <label class="text-xs uppercase font-bold text-slate-300 tracking-wider">Período Ágil:</label>
          <select 
            v-model="agilMode"
        ...
        <div v-if="agilMode === 'DIA'" class="flex flex-col gap-1">
          <label class="text-xs uppercase font-bold text-slate-300 tracking-wider">Seleccionar Día (de {{ agilSelectedMonth }}):</label>
          <select 
...
const agilSelectedMonth = ref(props.defaultMonth || 'JULIO')
...
const agilFilteredDates = computed(() => {
  const monthNum = props.monthToNumber[agilSelectedMonth.value]
