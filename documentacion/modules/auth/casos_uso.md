...
2. El guardián de rutas detecta que no hay sesión activa y renderiza la vista `LoginView.vue`.
3. El usuario ingresa su correo electrónico y su contraseña en el formulario y presiona **"Iniciar Sesión"**.
4. El frontend envía una solicitud `POST /api/auth/login` con el payload JSON conteniendo `correo` y `password`.
...
12. El frontend almacena el token en `localStorage.bimej12_auth_token`, actualiza el store `authStore` y redirige al Dashboard (`/`).
...
- **Secundario**: Cliente Frontend (Pinia, Vue Router).

### Precondiciones
- El usuario debe tener una sesión activa iniciada.

### Flujo Principal
1. El usuario hace clic en la opción **"Cerrar Sesión"** en la barra de navegación lateral.
2. El frontend ejecuta el método `logout()` en `authStore.ts`.
