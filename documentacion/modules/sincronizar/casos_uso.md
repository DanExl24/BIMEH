...
### Flujo Principal
1. El usuario accede a la vista de **Sincronización** (`src/features/sincronizar/views/SincronizarView.vue`).
2. En la pestaña "Archivo Local", selecciona el tipo (**Día** o **Mes**) y arrastra el archivo al componente `SyncFileDropzone.vue`.
3. El composable `useLocalFileUpload.ts` envía un formulario multipart a `POST /api/sincronizar/cargar` mediante `sync.service.ts` con `overwrite=false`.
...
7. El frontend muestra una notificación de éxito en color verde y actualiza el listado de fechas.
...
- **Paso 5**: Si una o más fechas ya tienen reportes en la base de datos y `overwrite` es `false`:
  ...
  4. Si elige sobrescribir, `useLocalFileUpload.ts` reenvía la petición con `overwrite=true`.
...
- **Paso 4**: Si faltan columnas requeridas:
  ...
  2. El frontend muestra una alerta roja y sugiere descargar la plantilla oficial desde `SyncTemplateDownload.vue`.
...
### Flujo Principal
1. El usuario selecciona la pestaña "Google Drive" en `SyncSourceSelector.vue`.
2. Selecciona el modo de sincronización (Día, Multi-Día con `SyncMultiDayCalendar.vue` o Mes Completo).
3. Presiona **"Iniciar Sincronización"**.
4. El frontend envía `POST /api/sincronizar/drive` mediante `sync.service.ts` con el JSON `DriveSyncRequest`.
...
8. El frontend muestra la consola de sincronización completada en `SyncDriveProgress.vue` y actualiza `dateStore.ts` con las nuevas fechas disponibles.
